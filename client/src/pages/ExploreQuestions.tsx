...
const ExploreQuestions = () => {
  ...
  const getQuestionPapers = async()=>{
    try{
       const response = await axiosInstance.get("/api/questions/papers",{
        headers:{
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {questionPapers.map((paper) => (
              <div className="group bg-white rounded-xl overflow-hidden border border-gray-200 hover:shadow-lg transition-all duration-300" key={paper.id}>
                ...
                <div className="p-5">
                  <h3 className="font-semibold text-lg text-gray-800 mb-4">{paper.subjectName}</h3>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-xs text-gray-500 mb-1">Year</p>
                      <p className="text-sm font-medium text-gray-900">{paper.year}</p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
                      <p className="text-xs text-gray-500 mb-1">Branch</p>
                      <p className="text-sm font-medium text-gray-900">{paper.branch}</p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-lg">
